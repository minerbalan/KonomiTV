...
// Fork機能: props の customSearch が変更されたら内部のcustomSearchを更新
watch(() => props.customSearch, (newCustomSearch) => {
    customSearch.value = { ...newCustomSearch };
}, { deep: true });
...
// Fork機能: VideoChoiceの選択が変更された時の処理
const handleVideoChoiceChange = (payload: { genre: string | null; date: string | null }) => {
    // カスタム検索条件を親コンポーネントに通知
    emit('update:customSearch', customSearch.value);
};
